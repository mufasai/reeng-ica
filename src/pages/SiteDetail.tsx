...
import { exportSiteReportToExcel } from '../utils/exportUtils';
import {
    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm px-6 py-4">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          {can('export_data') && (
            <button
              onClick={() => exportSiteReportToExcel(
                site,
                atpWorkOrders.filter(w => w.site_id === site.site_id),
                terminPengajuanRecords.filter(t => t.site_id === site.site_id),
                mockSiteFiles.filter(f => f.site_id === site.id || f.site_id === site.site_id),
                `site-${site.site_id}-report.xlsx`
              )}
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors"
